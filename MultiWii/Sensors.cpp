...
void i2c_rep_start(uint8_t address) {
    ...
    if (address & 0x01){
      ...
      #else
        //Wire.requestFrom((uint8_t)(address >>1) ,10);
        Wire.requestFrom(address >>1 ,10);
      #endif
    }else{
...
uint8_t i2c_read(uint8_t ack) {
  ...
  #else
    if (!ack) Wire.endTransmission(I2C_STOP);
  #endif
  return r;
}
...
void i2c_read_reg_to_buf(uint8_t add, uint8_t reg, uint8_t *buf, uint8_t size) {
  uint8_t *b = buf;
    ...
    *b = i2c_readNak();
    ...
    Wire.beginTransmission((uint8_t) add); 
    #if !defined (TEENSY3X)
      if (Wire.requestFrom ((uint8_t)add,(uint8_t)size,(uint8_t)true) == 0) i2c_errors_count++;
    #else
      //if (Wire.requestFrom ((uint8_t)add,size) == 0) i2c_errors_count++;
      if(Wire.requestFrom ((int)add,(uint8_t)size) == 0) i2c_errors_count++;
    #endif
